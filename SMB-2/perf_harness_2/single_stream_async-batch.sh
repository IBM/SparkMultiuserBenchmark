...
do 
	...
	do
    		...
    		$SPARK_HOME/bin/spark-submit --class com.ibm.platform.benchmarks.smb2.asyncbatch.AsyncBatchQueries --master $MASTER_NAME --deploy-mode cluster --driver-memory 3g --name UserAsyncQueryStream_$STREAMID_$CURRENTTIME AsyncBatchQueries.jar ${sequence[$r,$QUERYCOUNTER]} $TPCDSPATH $PERFDIR/ $STREAMID $SEQCOUNTER &  
		PID_SPARK_SUBMIT=$!
		((QUERYCOUNTER++))
		sleep $QUERYDELAY

                if [ "$MASTER_NAME" = "yarn" ]; then
