...
while [ $COUNTER -lt $NUM_BATCH_STREAMS ]; do
    ...
    ./single_stream_async-batch.sh $BATCH_NUM_SEQ_ITER $BATCH_QUERY_DELAY $BATCH_SEQ_DELAY $COUNTER $BATCH_TPCDS_DB_PATH > /perf_test/perf_data_2/driver_output_batch${COUNTER}.log 2>&1 &
...
while [ $COUNTER -lt $NUM_INT_STREAMS ]; do
    ...
    ./single_stream_sequential.sh $INT_NUM_SEQ_ITER $INT_QUERY_DELAY $INT_SEQ_DELAY $COUNTER $INT_TPCDS_DB_PATH > /perf_test/perf_data_2/driver_output_interactive${COUNTER}.log 2>&1 &
    let COUNTER=COUNTER+1
